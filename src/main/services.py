...
def extract_zip_file(zip_file_location, pk):
    with zipfile.ZipFile(zip_file_location, "r") as file_zip:
        ...
        zip_file_dir = os.path.join(os.path.dirname(zip_file_location), filename)
...
def get_pages_text(pdf_file, pages=3):
    ...
    if pages > 0:
        ...
        ocrmypdf.ocr(
            input_file=pdf_file,
            output_file=pdf_file + ".first_page.pdf",
            deskew=True,
            output_type="pdf",
            language="swe",
            skip_text=True,
    ...
    if bool(date):
        ...
        try:
            if date > datetime.now() or date.year < 2018:
                if not ignore_file:
                    return get_date_from_text(
                        text[text.index(temp) + len(temp) :],
                        ignore_file=True,
...
def get_organ_from_text(text):  # last
    ...
    if Organ.S.label in text and Organ.F.label in text:
        return (
            Organ.S.value
            if text.index(Organ.S.label) < text.index(Organ.F.label)
            else Organ.F.value
        )
