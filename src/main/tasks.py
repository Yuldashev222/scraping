            ...
            for year in years:
                ...
                try:
                    ...
                    for pdf_file in pdf_files:
                        print(pdf_file)
                        location = (
                            f"{directory_path}/{region}/{organ}/{year}/{pdf_file}"
                        )
                        ...
                        if file_format in "docxrtf":
                            services.convert_word_to_pdf(
                                normalize_location,
                                f"{directory_path}/{region}/{organ}/{year}",
                            )
                            file_format = (
                                file_format.replace("docx", "pdf")
                                .replace("doc", "pdf")
                                .replace("rtf", "pdf")
                            )
                            pdf_file = (
                                ".".join(pdf_file.split(".")[:-1]) + f".{file_format}"
                            )
                        ...
                        obj = models.FileDetail.objects.create(
                            ...
                            mode=(
                                FileMode.REGION
                                if model_region.endswith("_no")
                                else FileMode.KOMMUN
                            ),
                            is_active=True,
                            organ=model_organ,
...
def extract_url_pdf(webpage_url, inform_id):
    ...
    def threaded_extract(url, filename, view_file_name, retry=0):
        ...
        try:
            ...
            if "Exceeded" in str(e) and "redirects" in str(e):
                ...
                return
            retry += 1
            return threaded_extract(
                url=url, filename=filename, view_file_name=view_file_name, retry=retry
            )
        threads_running -= 1
    ...
    for a in soup.findAll("a"):
        ...
        if UnnecessaryFile.objects.filter(inform_id=inform.id, pdf_link=link).exists():
            ...
            continue
        elif models.IgnoreFile.objects.filter(
            link=webpage_url, source_file_link=link
        ).exists():
            ...
            if ignore_text:
                try:
                    ...
                    if dict(r_test.headers).get("Content-Type") == "application/pdf":
                        UnnecessaryFile.objects.get_or_create(
                            inform_id=inform.id, pdf_link=link
                        )
                except Exception as e:
                    print(e)
                continue
        ...
        try:
            ...
            if services.is_ignore_file(first_page_text, ignore_texts_from_first_page):
                ...
                try:
                    ...
                    if dict(r_test.headers).get("Content-Type") == "application/pdf":
                        UnnecessaryFile.objects.get_or_create(
                            inform_id=inform.id, pdf_link=pdf_link
                        )
                except Exception as e:
                    print(e)
            ...
            models.FileDetail.objects.create(
                text=text_in_file,
            ...
            try:
                if (
                    dict(requests.head(pdf_link).headers).get("Content-Type")
                    == "application/pdf"
                ):
                    UnnecessaryFile.objects.get_or_create(
                        inform_id=inform.id, pdf_link=pdf_link
...
def loop_links(start_inform_id):
    ...
    if start_inform_id == 0:
        max_inform_id = models.Inform.objects.aggregate(mx=models.models.Max("id"))[
            "mx"
        ]
        if max_inform_id:
            start_inform_id = max_inform_id
    informs = (
        models.Inform.objects.filter(id__lte=start_inform_id)
        .exclude(
            models.models.Q(id__in=[1058, 1057, 856, 855, 854, 853, 852])
...
def create_search_detail_obj(search_text, files_cnt, forwarded, real, remote):
    ...
    try:
        models.SearchDetail.objects.get_or_create(
            text=text, ipaddress=ip, result_files_cnt=files_cnt
        )
    except DataError:
        print(
            "create_search_detail_obj ------------------------------------------- bad text"
        )
