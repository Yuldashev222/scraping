...
class LogoAdmin(admin.ModelAdmin):
    ...
    def logo_img(self, obj):
        if obj.logo:
            return format_html(
                f"<a href='{obj.logo.url}'><img width=80 height=45 src='{obj.logo.url}'></a>"
            )
...
class RegionListFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        ...
        if country__exact:
            return (
                obj
                for obj in InformRegion.choices()
                if obj[0].startswith(country__exact)
            )
        return ((None, None),)
...
class FileDetailAdmin(admin.ModelAdmin):
    ...
    def link_id(self, obj):
        if obj.inform:
            return format_html(
                f"<a target='_blank' href='{obj.inform.link}'>{obj.inform.id}"
            )
        return None
    ...
    def delete_queryset(self, request, queryset):
        ...
        for obj in queryset:
            if (
                obj.inform
                and not models.IgnoreFile.objects.filter(
    ...
    def delete_model(self, request, obj):
        if (
            ...
            models.IgnoreFile.objects.create(
                link=obj.inform.link, source_file_link=obj.source_file_link
            )
        obj.delete()
...
class InformAdmin(admin.ModelAdmin):
    ...
    def delete_model(self, request, obj):
        if not obj.is_completed:
            self.message_user(
                request, _("the object being deleted is incomplete"), messages.ERROR
            )
            return
        obj.delete()
