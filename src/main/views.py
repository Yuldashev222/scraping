    ...
    _region_item = openapi.Schema(
        ...
        properties={
            ...
            "country": openapi.Schema(
                type=openapi.TYPE_STRING, description="Parent country code"
            ),
        },
    )
        ...
        responses={
            200: openapi.Response(
                ...
                schema=openapi.Schema(
                    type=openapi.TYPE_OBJECT,
                    properties={
                        "counties": openapi.Schema(
                            type=openapi.TYPE_ARRAY, items=_filter_item
                        ),
                        "municipalities": openapi.Schema(
                            type=openapi.TYPE_ARRAY, items=_region_item
                        ),
                        "organs": openapi.Schema(
                            type=openapi.TYPE_ARRAY, items=_filter_item
                        ),
                        "modes": openapi.Schema(
                            type=openapi.TYPE_ARRAY, items=_filter_item
                        ),
                    },
                ),
...
class SearchFilesView(ListAPIView):
    ...
    @swagger_auto_schema(
        ...
        responses={
            ...
            400: openapi.Response(
                description="Invalid filter parameter (mode, organ, region, country, or file_date)"
            ),
    ...
    def get(self, request, *args, **kwargs):
        ...
        if bol:
            ...
            if required_text_list:
                for required_text in required_text_list:
                    search = search.query(
                        self.exact_q_expression(required_text.replace('"', ""))
                    )
            ...
            if a_search_query:
                if len(a_search_query) > 1:
                    search = search.query(
                        self.all_q_expression(" ".join(a_search_query[:-1]))
                    )
        ...
        serializer = self.get_serializer_class()(
            response, many=True, context={"request": request, "bol": bol}
        )
